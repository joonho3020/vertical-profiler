...
/* auto access_info = generate_access_info(state.pc, FETCH, {false, false, false}); */
/* bool virt = access_info.effective_virt; */
    reg_t satp = this->get_state()->satp->readvirt(false);
    reg_t asid_mask_if_enabled = this->get_xlen() == 32 ? SATP32_ASID : SATP64_ASID;
    reg_t asid_mask = this->supports_impl(IMPL_MMU_ASID) ? asid_mask_if_enabled : 0;
    uint64_t PPNBITS = this->get_xlen() == 32 ? __builtin_popcount(SATP32_PPN) : __builtin_popcountll(SATP64_PPN);
    reg_t asid = (satp & asid_mask) >> PPNBITS;

    // FIXME: Add page table / asid info here?
    s << "core " << std::dec << std::setfill(' ') << std::setw(3) << id
      << std::hex << ": 0x" << std::setfill('0') << std::setw(max_xlen / 4)
      << zext(state.pc, max_xlen) << " (0x" << std::setw(8) << bits << ") "
      << disassembler->disassemble(insn) << " asid: " << asid << std::endl;
